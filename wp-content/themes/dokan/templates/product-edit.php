...
$_visibility = get_post_meta($post_id, '_visibility', true);
   ...
   $post_location= get_post_meta($post_id, 'location');									//****************************  DOKAN-CHILD ************************* 
...
<div class="tab-content">
	<div class="tab-pane fade in active" id="edit-product">
						...
						if($res)
						{
							foreach($res as $state)
							{
								...
								foreach ($res1 as $city) {
									echo "<option value='".strtolower($city)."' ".((in_array(strtolower($city),$post_location))?"selected":"").">$city</option>";
		...
		<div class="row">
			<div class="col-md-12">
				<?php wp_editor(esc_textarea($post->post_content), 'post_content', array('editor_height' => 50, 'quicktags' => false, 'media_buttons' => false, 'teeny' => FALSE, 'editor_class' => 'post_content', 'tinymce' => true)); ?>
			</div>
		</div>

<?php
   ...
   if (isset($_POST['add_product'])) {
     ...
     $post_location = array_map('trim',$_POST['post_location']);                                           //****************************  DOKAN-CHILD *************************
     ...
     if (empty($post_title)) {
       ...
       if (empty($_POST['post_location'])) {                                                        //****************************  DOKAN-CHILD *************************
       	$errors[] = __('Please enter product location', 'dokan');
       }
       ...
       if (!$errors) {
        ...
        dokan_child_process_location($product_id);									//****************************  DOKAN-CHILD *************************
				...
				<form class="form" method="post">
					...
					<div class="row product-edit-container">
						...
						<div class="col-md-4">
							...
							<div class="form-group">               <!--****************************  DOKAN-CHILD ************************* -->
								<select data-placeholder="Deal Location" style="width: 100%;" class="chosen-location" multiple="multiple" tabindex="-1" name="post_location[]">
									<option value="">All Locations</option>
									<?php  
									$res = $wpdb->get_col("
										SELECT DISTINCT city_state FROM ". $wpdb->prefix.'cities'." order by city_state");
									...
									if($res)
									{
										foreach($res as $state)
										{
											...
											$res1= $wpdb->get_col($wpdb->prepare("
												SELECT DISTINCT city_name from ".$wpdb->prefix.'cities'." where city_state='%s'",$state));
											if($res1)
												{echo "<optgroup label='$state'>";
											foreach ($res1 as $city) {
												echo "<option value='".strtolower($city)."' ".((isset($_POST['add_product']))? ((in_array(strtolower($city),$post_location))? "selected":""):"").">$city</option>";
					...
					<div class="form-group">
						<?php wp_editor($post_content, 'post_content', array('editor_height' => 50, 'quicktags' => false, 'media_buttons' => false, 'teeny' => false, 'editor_class' => 'post_content', 'tinymce' => true)); ?>
					</div>
